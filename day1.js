...
const main = () => {
    let ar = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
    
    let res = data.split("\n")
        .map((line) => {
            let start = getNum(line)[0];
            let last = getLast(line);

            ar.map((elem, i) => {
                if (elem == start) start = i + 1;
                if (elem == last) last = i + 1;
            });
            return "" + start + last;
        })
        .reduce((res, elem) => {
    ...
    function getNum(str) {
        return str.match(/one|two|three|four|five|six|seven|eight|nine|[0-9]/i);
    }

    function getLast(str) {
        for (let i = str.length, add = ""; i >= 0; i--) {
            add = str[i] + add;
            let t = getNum(add);
            if (t) return t[0];
        }
...
const data = `tmmnhlxzpj1eightldxhjnone97
