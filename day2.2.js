const main = () => {
    let reg = new RegExp("Game \\d*: ", "i");
    let regStr = new RegExp("[a-z]", "gi");
    let colors = ["red", "green", "blue"];

    console.log(
        data.split("\n")
            .reduce((sum, e) => {
                e = e = e.replace(reg, "").split(";");
                return sum + getMultRGB(e);
            }, 0)
    );

    function getMultRGB(ar) {
        let maxRGB = [0, 0, 0];

        ar.map(elem => {
            elem.split(",").map((elem, i) => {
                colors.map((color, j) => {
                    if ((elem.indexOf(color) > -1)) {
                        let num = +elem.replace(regStr, "");
                        if (num > maxRGB[j]) {
                            maxRGB[j] = num;
                        }
                    }
        ...
        return maxRGB.reduce((mult, elem) => mult * elem);
    }
};
